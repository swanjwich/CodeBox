...
const toggleDeletion = () => {
    if (snippetToDelete.value) {
        form.delete(`/snippets/${snippetToDelete.value.id}/destroy`, {
            preserveScroll: true,
            onSuccess: () => {
...
const toggleRestoration = (snippet) => {
    form.patch(`/snippets/${snippet.id}/restore`,{
        preserveScroll:true,
    })
}
    ...
    <div v-else>
        <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-4 mt-5">
            <div v-for="snippet in snippets" :key="snippet.id">
                <div class="shadow-sm hover:shadow-lg bg-gray-100 rounded-lg p-3 flex flex-col cursor-pointer">
                    <div class="flex justify-between">
                        <h5 class="font-semibold truncate">{{ snippet.title }}</h5>
                        <div class="relative">
                            <i @click="toggleDropdown(snippet.id)" class="ri-more-2-fill cursor-pointer"></i>
                            <div v-if="isDropdownOpen === snippet.id"
                                class="absolute right-0 mt-2 w-40 bg-white shadow-lg rounded-2xl p-2 z-50">
                                <ul>
                                    <li @click="toggleRestoration(snippet)"
                                        class="p-2 text-xs flex items-center gap-2 rounded-xl hover:bg-gray-200 cursor-pointer text-blue-500">
                                        <i class="ri-loop-left-line"></i> Restore
                                    </li>
                                    <li @click="confirmDeletion(snippet)"
                                        class="p-2 text-xs flex items-center gap-2 rounded-xl hover:bg-gray-200 cursor-pointer text-red-500">
                                        <i class="ri-delete-bin-fill"></i> Delete forever
                                    </li>
                                </ul>
